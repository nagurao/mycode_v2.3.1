  ...
  public:
	 ...
	 * @code
		void loop() {
			int sensorValue = analogRead(A0);
			float voltage = sensorValue * (5.0 / 1023.0);
			ThingSpeak.writeField(myChannelNumber, 1, voltage, 2, myWriteAPIKey); // 2 decimal points
			delay(20000);
		}
	 * @endcode
	 */
	int writeField(unsigned long channelNumber, unsigned int field, float value, unsigned int decimalPoints, const char * writeAPIKey)
	{
		#ifdef PRINT_DEBUG_MESSAGES
			Serial.print("ts::writeField (channelNumber: "); Serial.print(channelNumber); Serial.print(" writeAPIKey: "); Serial.print(writeAPIKey); Serial.print(" field: "); Serial.print(field); Serial.print(" value: "); Serial.print(value,5); Serial.println(")");
		#endif
		char valueString[20]; // range is -999999000000.00000 to 999999000000.00000, so 19 + 1 for the terminator
		int status = convertFloatToChar(value, valueString,(decimalPoints >=5) ? 5 : decimalPoints);
		if(status != OK_SUCCESS) return status;
...
private:
	...
	int convertFloatToChar(float value, char *valueString,int decimalPoints)
	{
		// Supported range is -999999000000 to 999999000000
		if(0 == isinf(value) && (value > 999999000000 || value < -999999000000))
		{
        ...
        #ifdef PARTICLE_PHOTONELECTRON
          //Photon doesn't have a dtostrf, but does have dtoa
          dtoa((double)value,decimalPoints, valueString);
        #else
		  dtostrf(value,1,decimalPoints, valueString);
        #endif
		return OK_SUCCESS;
	};
